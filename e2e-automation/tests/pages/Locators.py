...
class MainPageLocators(BaseLocators):
    ...
    SECTION_TITLES = (By.CSS_SELECTOR, "h3")
...
class InputComponentsLocators:
    ...
    NUMBER_INPUTS = (By.CSS_SELECTOR, "input[type='number']")
    TEXTAREAS = (By.CSS_SELECTOR, "textarea")
    
    # Валидация
    REQUIRED_INPUTS = (By.CSS_SELECTOR, "input[required], textarea[required]")
    INPUTS_WITH_MINLENGTH = (By.CSS_SELECTOR, "input[minlength], textarea[minlength]")
    INPUTS_WITH_MAXLENGTH = (By.CSS_SELECTOR, "input[maxlength], textarea[maxlength]")
...
class ButtonComponentsLocators:
    ...
    SUBMIT_BUTTONS = (By.CSS_SELECTOR, "button[type='submit']")
...
class ImageComponentsLocators:
    ...
    IMAGES_WITH_SRC = (By.CSS_SELECTOR, "img[src]")
...
class ModalComponentsLocators:
    ...
    VISIBLE_MODALS = (By.CSS_SELECTOR, "[role='dialog']:not([style*='display: none']), .modal:not([style*='display: none'])")
    HIDDEN_MODALS = (By.CSS_SELECTOR, "[role='dialog'][style*='display: none'], .modal[style*='display: none']")
...
class SliderComponentsLocators:
    ...
    SLIDERS_WITH_MIN = (By.CSS_SELECTOR, "input[type='range'][min]")
    SLIDERS_WITH_MAX = (By.CSS_SELECTOR, "input[type='range'][max]")
    SLIDERS_WITH_STEP = (By.CSS_SELECTOR, "input[type='range'][step]")
...
class IframeComponentsLocators:
    ...
    IFRAMES_WITH_SRC = (By.CSS_SELECTOR, "iframe[src]")
...
class NavigationLocators:
    """Локаторы для навигации между страницами"""
    
    # Ссылки на страницы
    HOME_PAGE_LINK = (By.CSS_SELECTOR, "a[href='/'], a[href='index.html']")
    LOGIN_PAGE_LINK = (By.CSS_SELECTOR, "a[href='login.html']")
    TESTS_PAGE_LINK = (By.CSS_SELECTOR, "a[href='tests.html']")
    
    # Кнопки навигации
    BACK_BUTTON = (By.CSS_SELECTOR, "button[onclick='history.back()'], .back-button")
    FORWARD_BUTTON = (By.CSS_SELECTOR, "button[onclick='history.forward()'], .forward-button")
    
    # Breadcrumbs
    BREADCRUMBS = (By.CSS_SELECTOR, ".breadcrumb, .breadcrumbs, nav[aria-label='breadcrumb']")


class ThemeLocators:
    """Локаторы для переключения тем"""
    
    # Кнопки переключения темы
    THEME_TOGGLE = (By.CSS_SELECTOR, ".theme-toggle, [test-id*='theme']")
    LIGHT_THEME_BUTTON = (By.CSS_SELECTOR, "[test-id='light-theme']")
    DARK_THEME_BUTTON = (By.CSS_SELECTOR, "[test-id='dark-theme']")
    
    # Индикаторы темы
    THEME_INDICATOR = (By.CSS_SELECTOR, "[data-theme]")


class LoadingLocators:
    """Локаторы для индикаторов загрузки"""
    
    # Спиннеры и индикаторы
    LOADING_SPINNER = (By.CSS_SELECTOR, ".spinner, .loading, .loader")
    PROGRESS_BAR = (By.CSS_SELECTOR, "progress, .progress-bar")
    
    # Состояния загрузки
    LOADING_STATE = (By.CSS_SELECTOR, "[aria-busy='true'], .loading-state")
    LOADED_STATE = (By.CSS_SELECTOR, "[aria-busy='false'], .loaded-state")
...
def get_locator(page_name: str, locator_name: str):
    ...
    if not hasattr(page_class, locator_name):
        raise AttributeError(f"Локатор '{locator_name}' не найден в классе '{page_class.__name__}'")
...
def get_all_locators_for_page(page_name: str):
    ...
    for attr_name in dir(page_class):
        if not attr_name.startswith('_') and not callable(getattr(page_class, attr_name)):
